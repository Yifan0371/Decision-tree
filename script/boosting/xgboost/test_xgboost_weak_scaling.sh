...
# ==============================================================================
# script/boosting/xgboost/test_xgboost_weak_scaling.sh
# XGBoost Weak-scaling test（自动按行截取同一大文件）
# ==============================================================================
...
LOGFILE="$SCRIPT_DIR/xgboost_weak_scaling_$(date +%Y%m%d_%H%M%S).log"
echo "日志文件: $LOGFILE"
echo "===============================================" >> "$LOGFILE"
echo "XGBoost Weak Scaling Performance Test Log" >> "$LOGFILE"
...
# 7) 打印结果表头
echo "==============================================="
echo "    XGBoost Weak Scaling Performance Test     "
echo "==============================================="
...
for t in "${threads[@]}"; do
    ...
    tmpfile="$PROJECT_ROOT/data/data_clean/tmp_xgboost_chunk_t${t}.csv"
    head -n "$lines_to_take" "$DATA" > "$tmpfile"

    # 8.3) 开始计时
    start_ts=$(date +%s%3N)

    # 8.4) 运行 XGBoost，并捕获完整 stdout
    output=$("$EXECUTABLE" \
        --data "$tmpfile" \
        --objective "$OBJECTIVE" \
